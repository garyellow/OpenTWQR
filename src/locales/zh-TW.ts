...
const zhTW = {
  ...
  common: {
    ...
    discardChangesDesc: '你尚未儲存的內容將會遺失。確定要離開這個畫面嗎？',
    discardChangesConfirm: '放棄變更',
    skipToMain: '跳至主要內容',
    goHome: '前往首頁',
    understand: '了解',
    loading: '載入中…',
  ...
  exportDialog: {
    ...
    catPrefsMode: '外觀模式',
    catPrefsLocale: '語系',
    catPaymentLinks: (n: number) => `轉帳 App 連結（${n} 個）`,
    passwordLabel: '設定密碼',
    passwordOptional: '（選填）',
    passwordPlaceholder: '不設定密碼則留空',
  ...
  /* ─── URL Scheme Settings ──────────────────────────────── */
  urlScheme: {
    sectionTitle: '轉帳 App 連動',
    infoTitle: '轉帳 App 連動',
    infoDesc: '掃描 QR Code 後，可自動開啟你設定的轉帳 App，並帶入帳號、金額等資訊，省去手動輸入。適合習慣使用特定轉帳 App 的使用者。',
    manageTitle: '轉帳 App 連動',
    entryRow: '管理轉帳 App',
    entryDesc: '設定轉帳 URL 與跨機構備援開啟方式',
    configCount: (n: number) => `${n} 間機構`,
    emptyTitle: '尚無設定',
    emptyHint: '設定連動後，掃描 QR Code 即可一鍵開啟轉帳 App。',
    addBank: '新增 App 連動',
    addLabel: '新增連動',
    editTitle: '編輯 App 連動',
    addTitle: '新增 App 連動',
    urlLabel: '轉帳 URL',
    transferUrlLabel: '轉帳 URL',
    transferUrlLabelShort: '轉帳 URL',
    urlPlaceholder: 'mybank://transfer?acct={account}',
    placeholderHelp: '轉帳 URL 變數',
    phAccount: '帳號',
    ...
    deleteTitle: '刪除 App 連動',
    deleteConfirm: '確定要刪除此 App 連動設定嗎？此操作無法復原。',
    save: '儲存',
    testLabel: '測試連動',
    testTitle: '測試 App 連動',
    testDesc: '填入測試資料，確認轉帳 URL 是否能正確開啟轉帳 App。',
    testBankCodeLabel: '機構代碼（選填）',
    testBankCodePlaceholder: '輸入收款方機構代碼（3 碼）',
    ...
    testOpen: '開啟測試連結',
    testAppHint: '若無法自動開啟，可能是此裝置未安裝對應 App，或轉帳 URL／備援 URL 有誤。',
    testLaunchUrlHint: '這個備援 URL 不會帶入帳號或金額，只在你限制「同機構才帶入資料」時，作為跨機構掃碼的開啟 App 入口。',

    // Same-institution-only
    sameInstitutionOnlyLabel: '僅在同機構時帶入資料',
    sameInstitutionOnlySummary: '限制轉帳 URL 只在掃到同機構收款碼時使用。',
    sameInstitutionOnlyHint: '開啟後，主轉帳 URL 只會在掃描到相同機構的 QR Code 時帶入帳號與金額。若你仍想在跨機構時顯示這個 App，可另外填寫下方的備援開啟 URL。',
    sameInstitutionBadge: '同機構帶入資料',
    allInstitutionsBadge: '所有機構皆可用',
    sameInstitutionNoFallbackHint: '跨機構掃碼時不顯示這個 App',
    launchUrlLabel: '跨機構備援開啟 URL（選填）',
    launchUrlLabelShort: '跨機構備援 URL',
    launchUrlBadge: '跨機構僅開啟 App',
    launchUrlPlaceholder: '跨機構時僅開啟 App 的連結…',
    launchUrlHint: '填寫後，跨機構掃碼時仍會顯示這個 App，但只會開啟 App，不會帶入帳號、金額或備註。留空則跨機構時不顯示。',
