...
interface ExportTaskContentProps {
  onCancel: () => void;
  onComplete: () => void;
  onDirtyChange?: (isDirty: boolean) => void;
}

export const ExportTaskContent = ({ onCancel, onComplete, onDirtyChange }: ExportTaskContentProps) => {
  ...
  return !result ? (
    <div className="space-y-6">
      <div className="space-y-2">
        <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider ml-1">
      ...
      <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
        {password ? t.exportDialog.infoWithPassword : t.exportDialog.infoWithoutPassword}
      </p>
    ...
    <div className="space-y-4">
      ...
      <button
        type="button"
        onClick={handleCopy}
        className="w-full py-4 rounded-xl font-semibold btn-accent active:scale-98 action-transition shadow-xs flex items-center justify-center gap-2 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-zinc-950"
      >
        {copied ? (
          <>
      ...
      {canShare && (
        <button
          type="button"
          onClick={handleShare}
          className="w-full py-4 rounded-xl font-semibold text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 active:scale-98 action-transition flex items-center justify-center gap-2 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-zinc-900 dark:focus-visible:ring-zinc-100 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-zinc-950"
        >
          {shared ? (
            <>
              <Check size={18} aria-hidden="true" className="animate-in zoom-in-75 fade-in duration-150 motion-reduce:animate-none" />
